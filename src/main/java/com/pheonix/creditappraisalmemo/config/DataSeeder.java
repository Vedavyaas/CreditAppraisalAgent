...
public class DataSeeder implements ApplicationRunner {
    ...
    private static final List<AppTemplate> TEMPLATES = List.of(
        new AppTemplate("TechNova Solutions Pvt Ltd",       "U72900KA2019PTC123456", "29AAACT1234A1Z5",
            "IT Services", "Bengaluru",        200,  5.2, false, "LOW",    "APPROVED",      85, "STRONG",    false, "credit@test.com"),
        new AppTemplate("Global Impex Traders",             "U51900MH2015PTC234567", "27AAACG5678B2Z6",
            "Import/Export", "Mumbai",          750,  18.4, true,  "HIGH",   "REJECTED",      42, "CONCERNING", true, "analyst@test.com"),
        new AppTemplate("Blue Ridge Manufacturing",         "U28900MH2010PTC345678", "27AAABC9012C3Z7",
            "Manufacturing", "Pune",            120,  12.1, false, "MEDIUM", "MANUAL_REVIEW", 61, "NEUTRAL",   false, "credit@test.com"),
        new AppTemplate("Aurelia Pharma Ltd",               "L24230MH2005PLC678901", "27AAAAP3456D4Z8",
            "Pharmaceuticals", "Hyderabad",     480,  4.8,  false, "LOW",    "APPROVED",      78, "STRONG",    false, "analyst@test.com"),
        new AppTemplate("Sunrise Agro Industries",          "U01100UP2012PTC789012", "09AAACS7890E5Z9",
            "Agribusiness", "Lucknow",           95,  22.6, true,  "HIGH",   "REJECTED",      35, "CONCERNING", true, "manager@test.com"),
        new AppTemplate("IndoCity Infrastructure Ltd",      "L45200DL2008PLC890123", "07AAAAI1234F6Z1",
            "Infrastructure", "Delhi",          1100, 8.5,  false, "MEDIUM", "MANUAL_REVIEW", 70, "NEUTRAL",   false, "credit@test.com"),
        new AppTemplate("Coastal Seafoods Export Pvt Ltd",  "U15400GJ2018PTC901234", "24AAAAC5678G7Z2",
            "Food Processing", "Surat",          320,  6.3,  false, "LOW",    "APPROVED",      80, "STRONG",    false, "analyst@test.com"),
        new AppTemplate("Nexgen Renewable Energy Pvt Ltd",  "U40100RJ2020PTC012345", "08AAAAN9012H8Z3",
            "Renewable Energy", "Jaipur",        560,  3.1,  false, "LOW",    "APPROVED",      90, "STRONG",    false, "credit@test.com"),
        new AppTemplate("Bharat Forge Dynamics",            "U29100PN2014PTC123890", "27AAABF1234K1Z0",
            "Heavy Industry", "Nagpur",          900,  14.2, false, "MEDIUM", "MANUAL_REVIEW", 75, "STRONG",    false, "manager@test.com"),
        new AppTemplate("Kaveri Textiles Pvt Ltd",          "U17100TN2011PTC098711", "33AAACK4567L1Z1",
            "Textiles", "Coimbatore",            180,  25.1, true,  "HIGH",   "REJECTED",      45, "NEUTRAL",   true, "analyst@test.com")
    );
    ...
    private void seedBank(Long appId, AppTemplate t, Random rng) {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            if (isCredit) tx.setCredit(bdOf(amount));
            else          tx.setDebit(bdOf(amount));
    ...
    private void seedMl(Long appId, AppTemplate t, Random rng) {
        double baseScore  = switch (t.riskBand()) { case "LOW" -> 78; case "HIGH" -> 28; default -> 54; };
        ...
        note.setSiteVisitObservations(
            t.capacityPct() >= 70
                ...
                : t.capacityPct() >= 50
                    ...
                    : "Only " + t.capacityPct() + "% capacity online. Multiple idle units. Dust accumulation on machinery suggests prolonged inactivity.");
    ...
    private void seedResearch(Long appId, AppTemplate t) {
        ...
        r.setMcaCin(t.cin());
        r.setMcaRegisteredDate("20" + (10 + (int)(Math.random() * 13)) + "-" + String.format("%02d", 1 + (int)(Math.random() * 12)) + "-01");
        r.setMcaPaidUpCapital("₹ " + (50 + (int)(Math.random() * 200)) + " Lakhs");
        r.setMcaDirectorCount(2 + (int)(Math.random() * 5));
        r.setMcaFilingStatus(t.riskBand().equals("HIGH") ? "Delayed (AOC-4 filed 45 days late)" : "Compliant (AOC-4 filed on time)");
        r.setMcaActive(true);
        ...
        r.setRbiPolicyNote("Repo rate at 6.5% — stable credit environment. MSME lending targets revised upward in FY25 budget.");
        r.setSectorGrowthPct(switch (t.industry()) {
            case "IT Services" -> 11.4; case "Pharmaceuticals" -> 9.8; case "Renewable Energy" -> 24.6;
            case "Food Processing" -> 7.3; case "Manufacturing" -> 8.1; case "Import/Export" -> 3.2;
            case "Infrastructure" -> 5.9; default -> 6.5;
        });
    ...
    private String buildNewsJson(String[] positive, String[] negative) {
        ...
        for (String h : positive) {
            if (idx > 0) sb.append(",");
            sb.append(String.format("{\"headline\":\"%s\",\"sentiment\":\"POSITIVE\",\"source\":\"%s\",\"date\":\"%s\"}",
                h.replace("\"", "'"), sources[idx % sources.length], dates[idx % dates.length]));
            idx++;
        }
        for (String h : negative) {
            sb.append(",");
            sb.append(String.format("{\"headline\":\"%s\",\"sentiment\":\"NEGATIVE\",\"source\":\"%s\",\"date\":\"%s\"}",
                h.replace("\"", "'"), sources[idx % sources.length], dates[idx % dates.length]));
            idx++;
        }
        sb.append("]");
    ...
    // ── Helpers ───────────────────────────────────────────────────────────────
    private static BigDecimal bdOf(double v) { return BigDecimal.valueOf(Math.round(v * 100)).movePointLeft(2); }
    ...
    private void seedScores(Long appId, AppTemplate t, Random rng) {
        ...
        int base = switch (t.riskBand()) { case "LOW" -> 85; case "HIGH" -> 45; default -> 65; };
