...
function buildFiveCs(mlPred: any, ddNote: any) {
    const score = mlPred?.riskScore ?? 50;
    ...
    return [
        ...
        { label: 'Capacity', icon: Wallet, grade: score >= 70 ? 'A' : score >= 50 ? 'B' : 'C', color: score >= 70 ? 'emerald' : score >= 50 ? 'amber' : 'red', detail: `ML risk score ${score}/100. Revenue: ${trend}.`, score },
...
export const CreditManagerDashboard: React.FC = () => {
    ...
    useEffect(() => { loadApp(selectedApp); }, [selectedApp, loadApp]);

    const riskScore = mlPred?.riskScore ?? report?.aiRiskScore ?? 50;
    const decision = mlPred?.decision ?? report?.autoDecision ?? 'MANUAL_REVIEW';
    ...
    const finalScore = Math.min(100, Math.max(0, riskScore + qualAdj));
                    ...
                    {dbApps.map(app => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <p className="text-xs font-bold text-slate-600 mt-1.5">Req: ₹ {app.requestedAmount ? (app.requestedAmount / 1e7).toFixed(1) : '—'} Cr</p>
                            </button>
                        );
                    })}
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
                ...
                <div className={`${dc.bg} rounded-2xl p-5 flex items-center justify-between shadow-md`}>
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            <p className="text-white/70 text-xs font-bold uppercase tracking-widest">AI Recommendation</p>
                    ...
                    <div className="xl:col-span-2 space-y-5">
                        ...
                        <MacWindow title="Why This Recommendation?">
                            <div className="p-4 space-y-2">
                                {(keys.length > 0 ? keys : [
                                    `Risk score ${riskScore}/100 — ${riskScore >= 70 ? 'low risk profile' : riskScore >= 50 ? 'medium risk' : 'high risk'}`,
                        ...
                        {mlPred?.forecastJson && (() => {
                            ...
                            return (
                                <MacWindow title="Revenue Forecast — Next 3 Months">
                                    <div className="p-4 flex items-end space-x-4 h-28">
                                        {vals.map((v: number, i: number) => (
                                            <div key={i} className="flex-1 flex flex-col items-center space-y-1">
                                                <span className="text-xs font-bold text-slate-600">₹{(v / 1e5).toFixed(1)}L</span>
                                                <div className="w-full bg-slate-100 rounded-lg flex flex-col justify-end" style={{ height: '64px' }}>
                                        ...
                                        <div className="text-xs text-slate-500 pl-2 border-l border-slate-200 pb-1">
                                            <p className="font-bold">{mlPred.revenueTrend}</p>
                                            <p className={mlPred.growthRatePct > 0 ? 'text-emerald-500' : 'text-red-500'}>{mlPred.growthRatePct > 0 ? '+' : ''}{mlPred.growthRatePct?.toFixed(1)}% MoM</p>
                                        </div>
                                    </div>
                                </MacWindow>
                            );
